...
- **Prior scan consumed:** [2026-05-30-business-scan.md](./2026-05-30-business-scan.md) merged on master (PR [#74](https://github.com/tienan92it/mindb/pull/74), tracker [#75](https://github.com/tienan92it/mindb/issues/75) closed). Product Planner digest [#77](https://github.com/tienan92it/mindb/issues/77) / [2026-05-30-daily-digest.md](./2026-05-30-daily-digest.md) deferred all four opportunities (top score 6/10); no ship candidates ≥ 7.
- **Trust cluster on master:** DDL schema cache (#35), schema failure warning (#39), row-cap notice (#43), partial schema index (#47), NL LLM error mapping (#53), safety-block transcript (#57), live settings refresh (#61), query timeout (#71). No feature merges since 2026-05-30 docs-only commits.
- **Pipeline:** Zero open `explore`, `planned`, `building`, `tech-reviewed`, or `ready-ship` issues; zero open PRs. Backlog needs Product Planner re-score on deferred execute/connect gaps.
- **Code review (master):** `SessionErrorMapper.mapExecuteFailure` handles read-only, cancel, timeout, and network cases but still falls through to `_shorten(error.toString())` for Postgres syntax/relation/permission failures (`session_error_mapper_test.dart` asserts unknown SQL errors shorten only). Agent `execute_sql` tool errors already route through `mapExecuteFailure`; `explain_sql` catch still uses raw `explain failed: $e` in `ai_agent_orchestrator.dart`. `ConnectionFormScreen._testConnection` sets `Connection failed: $e` while session `_connect` uses `SessionErrorMapper.map` (both still leave auth/SSL Postgres failures as shortened driver text). `AgentErrorEvent('Agent reached maximum tool rounds')` surfaces via `mapNlFailure` as a bare string with no recovery hint. `PostgresDatabaseClient.activeProfile` is not reconciled after connection-form saves — open sessions can run queries on stale TCP credentials until manual reconnect.
...
| Re-brief 2026-05-30 digest without code delta | No merges since digest; opps 1–4 unchanged on master — re-score, do not duplicate briefs. |
...
3. Opportunity 1 was 6/10 on [2026-05-30-daily-digest.md](./2026-05-30-daily-digest.md) — re-score first; scope: Postgres branches in `mapExecuteFailure` + wire `explain_sql` catch to same mapper; do not bundle timeout work.
...
**Inputs:** This file, `business-model.md`, `decisions.md`, prior scan [2026-05-30-business-scan.md](./2026-05-30-business-scan.md), digest [2026-05-30-daily-digest.md](./2026-05-30-daily-digest.md), `qa-smoke.md`.
